...
mk.contaOrdena = 0;
mk.status = {
    totalFull: _a.fullDados.length,
    totalFiltrado: _a.exibeDados.length,
    totalPorPagina: _a.exibePaginado.length,
    pagItensIni: 0,
    pagItensFim: 0,
    pagPorPagina: 5,
    totalPaginas: 0,
...
mk.Ao = (tipoEvento = "click", query, executar) => {
...
mk.QSetAll = (query = "input[name='#PROP#']", o = null) => {
    let eAfetados = [];
    if (o != null) {
...
mk.mkSelDlRefill = async (eName, cod = null) => {
    let e = mk.Q(eName);
...
mk.mkTrocaPontoPorVirgula = (query) => {
    mk.QAll(query).forEach((e) => {
        e.innerHTML = e.innerHTML.replaceAll(".", ",");
    });
};
mk.mkSelecionarInner = (e) => {
...
mk.mkInputFormatarValor = (e) => {
    e.value = mk.mkDuasCasas(mk.mkFloat(e.value));
};
mk.mkMedia = (menor, maior) => {
...
mk.mkBase64 = (arquivo, tagImg, tagHidden) => {
...
mk.encheArray = (arrTemplate, inicio = 1, total) => {
    let novaArray = [];
...
mk.encheArrayUltimos = (arrTemplate, fim = 1, total) => {
    let novaArray = [];
    if (Array.isArray(arrTemplate)) {
        if (arrTemplate.length > 0) {
...
mk.getDiasDiferenca = (msOld, msNew = null) => {
    if (msNew == null)
        msNew = mk.getMs();
    return mk.transMsEmDias(msNew - msOld);
};
mk.transMsEmSegundos = (ms) => {
    return Math.trunc(ms / 1000);
};
mk.transMsEmMinutos = (ms) => {
    return Math.trunc(ms / 60000);
};
mk.transMsEmHoras = (ms) => {
    return Math.trunc(ms / 3600000);
};
mk.transMsEmDias = (ms) => {
    return Math.trunc(ms / 86400000);
};
mk.transSegundosEmMs = (s) => {
    return s * 1000;
};
mk.transMinutosEmMs = (m) => {
    return m * 60000;
};
mk.transHorasEmMs = (h) => {
    return h * 3600000;
...
mk.mkYYYYMMDDtoDDMMYYYY = (dataYYYYMMDD) => {
...
mk.http = async (url, metodo = mk.t.G, tipo = mk.t.J, dados = null, carregador = false) => {
...
mk.getObjetoFromId = (nomeKey, valorKey, listaDados) => {
    let temp = null;
...
mk.setObjetoFromId = (nomeKey, valorKey, itemModificado, listaDados) => {
    if (Array.isArray(listaDados)) {
        listaDados.forEach((o) => {
            if (o[nomeKey] == valorKey) {
                o = itemModificado;
...
mk.delObjetoFromId = (nomeKey, valorKey, listaDados) => {
    let temp = [];
    if (Array.isArray(listaDados)) {
        listaDados.forEach((o) => {
            if (o[nomeKey] != valorKey) {
...
mk.atualizarStatusLista = () => {
    ...
    mk.status.totalFull = _a.fullDados.length;
    mk.status.totalFiltrado = _a.exibeDados.length;
    mk.status.totalPorPagina = _a.exibePaginado.length;
    mk.status.pagItensIni =
        (_a.paginationAtual - 1) * mk.status.pagPorPagina + 1;
    ...
    mk.status.totalPaginas = Math.ceil(_a.exibeDados.length / mk.status.pagPorPagina);
};
mk.ativaPaginaAtual = () => {
    ...
    mk.QAll(".paginate_button .page-link").forEach((item) => {
        if (_a.paginationAtual == Number(item.innerHTML)) {
...
mk.filtraPagination = () => {
    ...
    if (_a.paginationAtual < 5) {
        ...
        if (mk.status.totalPaginas - _a.paginationAtual < 4) {
            ...
            mk.Q(".tablePaginacao .pageCod2").classList.add("disabled");
            mk.Q(".tablePaginacao .pageCod2 a").innerHTML = "...";
            mk.Q(".tablePaginacao .pageCod3 a").innerHTML = (_a.paginationAtual - 1).toString();
            mk.Q(".tablePaginacao .pageCod4 a").innerHTML =
                _a.paginationAtual.toString();
            mk.Q(".tablePaginacao .pageCod5 a").innerHTML = (_a.paginationAtual + 1).toString();
            mk.Q(".tablePaginacao .pageCod6 a").innerHTML = "...";
            mk.Q(".tablePaginacao .pageCod6").classList.add("disabled");
        }
    }
    ...
    _a.exibeDados.forEach((item, i) => {
        if (i + 1 >= mk.status.pagItensIni && i + 1 <= mk.status.pagItensFim) {
            ...
            _a.exibePaginado.push(objItem);
        }
    });
};
mk.mkFiltragemDados = () => {
    if (Array.isArray(_a.fullDados)) {
        let temp = [];
        _a.fullDados.forEach((o) => {
            let podeExibir = true;
            for (let propFiltro in mk.objFiltro) {
                if (o[propFiltro] != null) {
                    let m = o[propFiltro];
                    let k = _a.objFiltro[propFiltro];
                    if (k.formato === "string") {
            ...
            if (podeExibir) {
                if (_a.objFiltro["mkFullFiltro"]) {
                    let k = _a.objFiltro["mkFullFiltro"]["conteudo"]
                        .toString()
                        .toLowerCase();
                    podeExibir = false;
...
mk.mudaPagina = (e) => {
    if (typeof e == "string") {
        ...
        _a.paginationAtual = Number(e.innerHTML);
    }
    mk.atualizarLista();
};
...
mk.mkGerarFiltro = (e) => {
    ...
    if (_a.objFiltro[e.name]["conteudo"] == "" ||
        _a.objFiltro[e.name]["conteudo"] == "0" ||
        _a.objFiltro[e.name]["conteudo"] == 0 ||
        _a.objFiltro[e.name]["conteudo"] === null) {
        delete _a.objFiltro[e.name];
    }
    mk.atualizarPorPagina();
};
mk.mkUpdateFiltro = () => {
...
mk.iniciarGetList = async (url) => {
    ...
    if (retorno != null) {
        ...
        _a.fullDados = _a.exibeDados = retorno;
        mk.ordenarDados();
        mk.mkUpdateFiltro();
    }
};
mk.adicionarDados = (objDados) => {
    _a.fullDados.push(mk.aoReceberDados(objDados));
    mk.ordenarDados();
    mk.atualizarLista();
};
mk.editarDados = (objDados, nomeKey, valorKey) => {
    _a.fullDados = mk.delObjetoFromId(nomeKey, valorKey, _a.fullDados);
    _a.fullDados.push(mk.aoReceberDados(objDados));
    mk.ordenarDados();
    mk.atualizarLista();
};
mk.excluirDados = (nomeKey, valorKey) => {
    _a.fullDados = mk.delObjetoFromId(nomeKey, valorKey, _a.fullDados);
    mk.ordenarDados();
    mk.atualizarLista();
};
mk.mkSetFiltroListener = () => {
    mk.QAll("input.iConsultas").forEach((e) => {
...
mk.LimparFiltro = (form = "#consulta_form") => {
    _a.objFiltro = {};
    let eForm = mk.Q(form);
    eForm.reset();
...
mk.LimparFiltroUpdate = (form = "#consulta_form") => {
    mk.LimparFiltro(form);
    mk.atualizarLista();
};
mk.ordenar = (array = _a.fullDados, nomeProp = _a.sortBy, reverse = false) => {
    array.sort((oA, oB) => {
...
mk.ordenarDados = () => {
    ...
    if (_a.sortDir == "d") {
        _a.fullDados = _a.fullDados.reverse();
    }
    ...
    let thsSort = mk.QAll(".sort-" + _a.sortBy);
    if (thsSort.length != 0) {
...
mk.inverteDir = (ordenar = null) => {
    if (ordenar != null) {
        if (ordenar != _a.sortBy) {
            _a.sortDir = "a";
        }
        else {
            _a.sortDir == "a" ? (_a.sortDir = "d") : (_a.sortDir = "a");
        }
        _a.sortBy = ordenar;
    }
    mk.ordenarDados();
...
mk.GerarAoSort = (trHeadPai = "table.tableListagem thead tr") => {
...
mk.mkValidaFull = (form, fUIValidou, varRepassaA = null) => {
    mk.mkCountValidate++;
...
mk.fUIFaseUpdateView = (obj) => {
    ...
    _a.mkFaseAtual = obj["destinoFase"];
    mk.Q(".modalFase" + _a.mkFaseAtual).classList.remove("oculto");
...
mk.fUIFaseVoltar = (esteForm) => {
    let obj = {
        destinoFase: _a.mkFaseAtual - 1,
        form: esteForm,
    };
    mk.fUIFaseUpdateView(obj);
};
mk.fUIFaseAvancar = (esteForm) => {
    let obj = {
        destinoFase: _a.mkFaseAtual + 1,
        form: esteForm,
    };
...
mk.fUIFaseEspecifica = (e) => {
    let obj = {
        destinoFase: Number(e.getAttribute("data-pag")),
        form: mk.getFormFrom(e),
    };
    if (obj.destinoFase < _a.mkFaseAtual ||
        e.getAttribute("data-libera") == "true") {
        mk.mkValidaFull(obj.form, mk.fUIFaseLiberarView, obj);
    }
};
mk.fUIFaseLiberarView = (obj) => {
    _a.sendObjFull = mk.mkGerarObjeto(obj["form"]);
    mk.fUIFaseUpdateView(obj);
};
mk.mkClicarNaAba = (este) => {
    if (este != null) {
...
mk.mkExibirModalFull = async (url = null, modelo) => {
...
mk.mkAModal = async (url = null, modelo = null, conteudo = null) => {
