...
const mkSelRenderizar = () => {
	document.querySelectorAll("input.mkSel").forEach((e) => {
		...
		if (!e.parentElement.classList.contains("mkSelBloco")) {
			let bArrows = e.getAttribute("data-selmoversel");
			// COLETA
...
const mkSelLeftSel = (e) => {
	let achou = false;
	let eSelecionado = null;
	Array.from(e.parentElement.nextElementSibling.children).forEach((el) => {
		if (el.getAttribute("data-s") == "1") {
			achou = true;
		}
		if (achou) {
			eSelecionado = el.previousElementSibling;
			achou = false;
		}
	});
	if (eSelecionado != e.parentElement.nextElementSibling.firstElementChild) {
		mkSelSelecionar(eSelecionado);
	} else {
		mkSelSelecionar(
			e.parentElement.nextElementSibling.lastElementChild.previousElementSibling
		);
	}
};
...
const mkSelRightSel = (e) => {
	...
	if (eSelecionado != null) {
		...
		mkSelSelecionar(
			e.parentElement.nextElementSibling.firstElementChild.nextElementSibling
		);
	}
};
...
/*
	...
	- Mobile: O filtro ficou oculto atraz da lista. (O Filtro e os controles precisam subir junto e ficarem organizaveis e visiveis)
	-- Uma ideia boa é fazer um translate e redimencionar o campo para o topo absoluto junto com a lista.
	-- Trocar o top e left da lista e dos controles
	-- Inclui uma camada de cancel atraz do seletor para sair do seletor sem clicar em alguma outra distração da pagina.
	- Verficar a possibilidade de  pressionar ESC ou botão VOLTAR do mobile para desfocar.
	-> Controles: Seletores laterais
*/
