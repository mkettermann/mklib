...
const mkSelGetKV = (e) => {
	if (mkIsJson(e.value)) {
		let kSels = JSON.parse(e.value);
		if (!Array.isArray(kSels)) {
			kSels = [kSels];
		}
		if (mkIsJson(e.getAttribute("data-selarray"))) {
			let kOpcoes = JSON.parse(e.getAttribute("data-selarray"));
			if (!Array.isArray(kSels)) {
				kOpcoes = [kOpcoes];
			}
			let res = [];
			kSels.forEach((k) => {
				...
				kOpcoes.forEach((opcao) => {
					opcao.k == k ? (obj.v = opcao.v) : null;
				});
				res.push(obj);
			});
			return res;
		}
		// Quando selecionar 2 e não tem a lista de opções
		let res = [];
		JSON.parse(e.value).forEach((kSel) => {
			res.push({ k: kSel.toString() });
		});
		return res;
	}
	return [{ k: e.value }];
