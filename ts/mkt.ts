...
class mkt {
		...
		if (e.value != null && e.getAttribute("data-mkfignore") != "true") {
			...
			if (tipo == "date") {
				...
				if (eData.length >= 2) {
					...
					if (eData[0]?.value != "") this.c.objFiltro[e.name].conteudo = eData[0]?.value;
					if (eData[1]?.value != "") this.c.objFiltro[e.name].conteudo2 = eData[1]?.value;
				}
			}
		}
	...
	static cte = (s: any, config: any = null, ...rest: any) => {
		...
		try {
			...
			if (!config.pacote.ok) {
				...
				if (!config.quiet) {
					...
					mkt.gc(
						`%cRetorno %c${config.pacote.status} %c(${config.metodo}) %c${tipo} %c${tam} ${config._url.pathname}`,
	...
	static mkConfirma = async (
		...
		let emSim = "Sim";
		if (p?.emSim != undefined) emSim = p.emSim;
		let emNao = "N&#227;o";
		if (p?.emNao != undefined) emNao = p.emNao;
		let titulo = "Confirmação";
		if (p?.titulo != undefined) titulo = p.titulo;
		let fnFora = () => { return mkt.w("Selecione apenas as opções da tela."); };
		if (p?.fnFora != undefined) fnFora = p.fnFora; // Overwrite fn.
		let classContainer = "";
	...
	static svgToImg = async (foreignObject: string, formatoDestino = "png", largura = 800, altura = 600, qualidade = 1.0) => {
		...
		if (eImg) {
			...
			eCanvas?.getContext('2d')?.drawImage(eImg as HTMLImageElement, 0, 0);
	...
	static encod = (input: string) => {
		...
		let nums2 = mkt.convertToNumeric(nums.reverse().join());
	...
	static decod = (input: string) => {
		...
		let nums = mkt.convertToTexto(nums2).split(",").reverse().map(i => Number(i));
...
<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' style='-webkit-box-reflect: right;' viewBox='0 0 16 16'><path fill-rule='evenodd' d='M8 1a.5.5 0 0 1 .5.5v11.793l3.146-3.147a.5.5 0 0 1 .708.708l-4 4a.5.5 0 0 1-.708 0l-4-4a.5.5 0 0 1 .708-.708L7.5 13.293V1.5A.5.5 0 0 1 8 1'/></svg>
	...
	async maisLinhas(inicio: number, total: number) {
		...
		if (this.config.selapenas != 1 || this.config.scrollcharge == false) {
			Array.from(this.config.selecionados.keys()).forEach((k: any) => {
